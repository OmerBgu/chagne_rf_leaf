...
def _parallel_build_trees_GaussianNB(tree, forest, X, y, sample_weight, tree_idx, n_trees,
                          verbose=0, class_weight=None):
    """Private function used to fit a single tree in parallel."""
    if verbose > 1:
        print("building tree %d of %d" % (tree_idx + 1, n_trees))
    ...
    if forest.bootstrap:
        ...
        tree.fit_GaussianNB(X, y, sample_weight=curr_sample_weight, check_input=False)
    else:
        tree.fit_GaussianNB(X, y, sample_weight=sample_weight, check_input=False)
